...
function DonutChart(props) {
    ...
    useEffect(()=>{
        setData(
            {
                labels: [
                    `Active ${props.data.cases_time_series[props.data.cases_time_series.length-1].dailyconfirmed}`,
                    `Deceased ${props.data.dailydeceased}`,
                    `Recovered ${props.data.dailyrecovered}`
                ], 
                datasets: [{
                    data: [9213, 999, 31233],
                    backgroundColor: [
